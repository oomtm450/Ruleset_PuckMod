...
using System.Text;
using static Codebase.WeightErrorHandlingType;
    ...
    public class LockWeightedList<T> : IEnumerable<T> {
        ...
        private void Recalculate() {
            ...
            while (small.Count > 0 && large.Count > 0) {
                int smallLastIndex = small.Count - 1;
                int l = small[smallLastIndex]; // 5.1
                small.RemoveAt(smallLastIndex);

                int largeLastIndex = large.Count - 1;
                int g = large[largeLastIndex]; // 5.2
                large.RemoveAt(largeLastIndex);
            ...
            while (large.Count > 0) {
                int largeLastIndex = large.Count - 1;
                int g = large[largeLastIndex]; // 6.1
                large.RemoveAt(largeLastIndex);
